...
--
-- Host: localhost    Database: CS_204_Assignment
-- ------------------------------------------------------
...
--
-- Current Database: `CS_204_Assignment`
--

CREATE DATABASE /*!32312 IF NOT EXISTS*/ `CS_204_Assignment` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;

USE `CS_204_Assignment`;
...
CREATE TABLE `Authors` (
  ...
  `AuthorNationality` varchar(50) DEFAULT NULL,
...
INSERT INTO `Authors` VALUES (1,'Sofia','Smith','Canada'),(2,'Maria','Brown','Brazil'),(3,'Elena','Martin','Mexico'),(4,'Zoe','Roy','France'),(5,'Sebastian','Lavoie','Canada'),(6,'Dylan','Garcia','Spain'),(7,'Ian','Cruz','Mexico'),(8,'Lucas','Smith','USA'),(9,'Fabian','Wilson','USA'),(10,'Liam','Taylor','Canada'),(11,'William','Thomas','Great Britain'),(12,'Logan','Moore','Canada'),(13,'Oliver','Martin','France'),(14,'Alysha','Thompson','Canada'),(15,'Isabelle','Lee','Canada'),(16,'Emily','Clark','USA'),(17,'John','Young','China'),(18,'David','Wright','Canada'),(19,'Thomas','Scott','Canada'),(20,'Helena','Adams','Canada'),(21,'Sofia','Carter','USA'),(22,'Liam','Parker','Canada'),(23,'Emily','Murphy','USA');
...
CREATE TABLE `Books` (
  ...
  `BookTitle` varchar(35) DEFAULT NULL,
  ...
  CONSTRAINT `books_ibfk_1` FOREIGN KEY (`BookAuthor`) REFERENCES `Authors` (`AuthorID`)
...
CREATE TABLE `Borrowers` (
  ...
  KEY `BookID` (`BookID`),
  KEY `BorrowID` (`BorrowID`,`ClientID`,`BookID`,`BorrowDate`),
  CONSTRAINT `borrowers_ibfk_1` FOREIGN KEY (`ClientID`) REFERENCES `Clients` (`ClientID`),
  CONSTRAINT `borrowers_ibfk_2` FOREIGN KEY (`BookID`) REFERENCES `Books` (`BookID`)
...
CREATE TABLE `Clients` (
  ...
  `ClientFirstName` varchar(25) DEFAULT NULL,
  `ClientLastName` varchar(25) DEFAULT NULL,
  `ClientDOB` year DEFAULT NULL,
  `Occupation` varchar(50) DEFAULT NULL,
  PRIMARY KEY (`ClientID`),
  KEY `ClientID` (`ClientID`,`ClientFirstName`,`ClientLastName`,`ClientDOB`,`Occupation`)
...
INSERT INTO `locations` VALUES (1,1,'Independence Hall',794.35,651.43),(2,2,'6709 Wonder Street',323.41,412.22),(3,1,'Sunrise',221.45,132.43),(4,2,' 356 A Street',123.32,222.43),(5,1,'Mountains',34.12,87.99),(6,2,'900 Star Street',1071.9,206.45),(7,1,'Moonrise',816.2,111.2),(8,2,'183714 N North Street',176.11,11.176);
...
CREATE TABLE `photographs` (
  ...
  `userid` int NOT NULL,
  UNIQUE KEY `photo_id` (`photoid`),
  KEY `userid` (`userid`),
  CONSTRAINT `photographs_ibfk_1` FOREIGN KEY (`userid`) REFERENCES `users` (`userid`)
...
INSERT INTO `photographs` VALUES (1,2,1),(2,2,1),(3,6,3),(4,8,4);
...
CREATE TABLE `users` (
  ...
  CONSTRAINT `users_chk_1` CHECK (((`zip` >= 10000) and (`zip` <= 99999)))
...
INSERT INTO `users` VALUES (1,'Bonnie Buntcake','bbunt','6709 Wonder Street','Wonderbread','OH',46106),(2,'Sam Smarf','ssmarf','356 A Street','Beefy','PA',19943),(3,'Wendy Grog','wgrog','900 Star Street','Mary','MD',21340),(4,'Joe Jogger','jjogger','183713 N North Street','Norther','WV',51423);
...
--
-- Final view structure for view `popularborrowed`
--
...
/*!50001 VIEW `popularborrowed` AS select `books`.`BookTitle` AS `BookTitle`,count(distinct `clients`.`ClientID`) AS `BorrowedClients` from ((`books` join `borrowers` on((`borrowers`.`BookID` = `books`.`BookID`))) join `clients` on((`borrowers`.`ClientID` = `clients`.`ClientID`))) group by `books`.`BookTitle` having (count(distinct `borrowers`.`ClientID`) >= (select (count(0) * 0.2) from `clients`)) order by `BorrowedClients` */;
...
-- Dump completed on 2024-04-26 23:24:12
